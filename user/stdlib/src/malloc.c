...
struct marker;
extern const struct marker heap_start;

static const size_t minimumAllocSize = 1; // 2 * sizeof(List);

extern memlimits_t __memoryLimits;
...
#endif
{
	...
	while((cursor != NULL) && ((cursor->used != 0) || (cursor->length < (len + sizeof(List))))) {
		cursor = cursor->next;
	}
	if(cursor == NULL) {
#if defined(ENABLE_MALLOC_MONITORING)
		...
		print_list();
#endif
		die_extra("malloc.OutOfMemory", itoa(len, NULL, 10));
	}
...
void free(void *ptr)
{
	...
	if((uintptr_t)ptr < (uintptr_t)malloc_heap_start) {
		die_extra("free.InvalidFree", itoa((int)ptr, NULL, 16));
	}
...
#if defined(USE_MAGIC_SECURED_MALLOC)
	if(isValid(entry) == 0) {
		die_extra("free.InvalidBlockMagic: ", itoa(entry->hash, NULL, 16));
	}
#endif
	...
	if(entry->length > 0x5000) {
		die_extra("free.InvalidSizedBlock: ", itoa(entry->length, NULL, 10));
	}
...
#endif
	/*
	serial_write_str(SERIAL_COM1, itoa((int)ptr, NULL, 16));
	serial_write_str(SERIAL_COM1, "\n");
	*/
...
void free_d(void *ptr, const char *file, int line)
{
	...
	serial_write_str(SERIAL_COM1, itoa(entry->length, NULL, 10));
	serial_write_str(SERIAL_COM1, " bytes at ");
	serial_write_str(SERIAL_COM1, itoa((int)ptr, NULL, 16));
